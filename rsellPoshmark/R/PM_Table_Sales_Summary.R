#' Creates a Poshmark Sales Summary Table
#'
#' This function creates a Poshmark Sales Summary Table from the Inventory Table.
#'
#' Requires: dplyr, scales
#'
#' @param pm_sales The Poshmark Sales Table for the user.
#' @return A Poshmark Sales Summary Table as a data frame.
#' @export
#' @examples
#'

PM_Table_Sales_Summary <- function(pm_sales) {

    sales_summary <- pm_sales %>%

                     mutate(net_profit  = net_earnings - item_cost,
                            days_listed = as.Date(order_date) - as.Date(listing_date)) %>%

                     summarize(total_items_sold = n(),
                               last_settle      = substr(max(pm_sales$date_add_db), 1, 10),
