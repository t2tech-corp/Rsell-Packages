...
Process_Valid_Report <- function(t_datapath) {
    ...
    sales_activity <- sales_activity %>% select(user_id,          listing_date,  order_date,      days_listed,      order_id,      sku,
    ...
    sales_activity <- Fix_Partial_SKU(sales_activity)
    sales_activity <- Fix_Blank_SKU(sales_activity)

    #### Get Cost Data and Update Profit ####

    sales_activity <- Get_Item_Cost(sales_activity)
